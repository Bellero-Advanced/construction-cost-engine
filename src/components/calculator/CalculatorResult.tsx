...
export function CalculatorResult({ data }: { data: CalcResult }) {
  ...
  return (
    <div className="page-in print-result">
      {/* Print-only header (hidden on screen) */}
      <div className="hidden print:mb-6 print:block print:border-b-2 print:border-ink print:pb-3">
        <div className="font-display text-[28px] tracking-[0.02em]">
          Construction Cost Engine — BOM Report
        </div>
        <div className="mt-1 grid grid-cols-2 gap-x-6 gap-y-1 font-mono text-[11px] text-ink-2">
          <div>
            <strong>หมวดงาน:</strong> {data.workName}
          </div>
          <div>
            <strong>วันที่:</strong> {new Date().toLocaleString("th-TH")}
          </div>
          <div>
            <strong>แหล่งราคา:</strong> {src.name}
          </div>
          <div>
            <strong>จังหวัด:</strong> {prov.name}
          </div>
          <div className="col-span-2">
            <strong>ข้อมูลคำนวณ:</strong> {data.extraInfo}
          </div>
        </div>
      </div>

      {/* Summary */}
      <div className="mb-5 grid gap-4 md:grid-cols-3 print:hidden">
        <SumCard
      ...
      <div className="doc mb-6" data-print-area>
        ...
        {/* Print-only unit cost row */}
        <div className="hidden print:mt-4 print:flex print:justify-between print:border-t-2 print:border-ink print:pt-3 print:font-mono print:text-[12px]">
          <span>
            <strong>UNIT COST:</strong> {fmt(data.unitCost)} {data.unitLabel}
          </span>
          <span className="text-ink-3">
            Generated by construction-cost-engine.workers.dev
          </span>
        </div>
      </div>

      {/* Breakdown */}
      <div className="doc mb-6 print:hidden">
